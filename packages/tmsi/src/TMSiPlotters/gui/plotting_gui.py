...
from apex_sdk.device.tmsi_device_enums import (
    MeasurementType as ApexMeasurementType,
)
...
class PlottingGUI(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(
        ...
        if isinstance(device, TMSiDevice):
            if not channel_selection:
                self._channel_selection = np.arange(
                    0, np.size(self.device.get_device_channels(), 0) - 1
                )
            else:
                ...
                self._channel_selection = np.hstack(
                    (
                        channel_selection,
                        np.size(self.device.get_device_channels(), 0) - 2,
                    )
                )
        else:
            if not channel_selection:
                self._channel_selection = np.arange(
                    0, np.size(self.device.channels, 0) - 1
                )
                ...
                for i in channel_selection:
                    ...
                    if i == np.size(self.device.channels, 0) - 2:
                        ...
                        channel_selection = channel_selection[:_idx]
                self._channel_selection = np.hstack(
                    (channel_selection, np.size(self.device.channels, 0) - 2)
                )
        ...
        if plotter_format == PlotterFormat.signal_viewer:
            self.real_time_plot = SignalViewer(
                self, device, filter_app=filter_app, grid_type=self.grid_type
            )
            ...
            self.real_time_plot = HeatMapViewer(
                gui_handle=self,
                device=device,
            ...
            self.set_range_box.activated.connect(
                lambda x: self.real_time_plot._update_scale("range")
            )
            self.autoscale_button.clicked.connect(
                lambda x: self.real_time_plot._update_scale("scale")
            )

            self.startEvent()

        elif plotter_format == PlotterFormat.impedance_viewer:
            self.real_time_plot = ImpedanceViewer(
                gui_handle=self,
                device=device,
    ...
    def initUI(self):
        ...
        for i in range(self.active_channels - 2):
            _checkBox = QtWidgets.QCheckBox(
                self.device.channels[
                    self.active_channel_conversion_list[i]
                ].name
            )
            if i in self._channel_selection:
                _checkBox.setChecked(True)
            self._gridbox.addWidget(_checkBox, i % 25, np.floor(i / 25))
            _checkBox.clicked.connect(
                self.real_time_plot._update_channel_display
            )
        ...
        self.increase_time_button.clicked.connect(
            self.real_time_plot._increase_time_range
        )
        self.decrease_time_button.clicked.connect(
            self.real_time_plot._decrease_time_range
        )
        self.set_range_box.activated.connect(
            lambda x: self.real_time_plot._update_scale("range")
        )
        ...
        self.increase_time_button.setText(
            "Increase time range: "
            + str(self.real_time_plot.window_size + 1)
            + "s"
        )
        ...
        self.show_DIGI_button.clicked.connect(
            self.real_time_plot._show_all_DIGI
        )
        self.hide_DIGI_button.clicked.connect(
            self.real_time_plot._hide_all_DIGI
        )

        if self.filter_app:
            self.disable_filter_button.clicked.connect(
                self.filter_app.disableFilter
            )
            self.enable_filter_button.clicked.connect(
                self.filter_app.enableFilter
            )
        else:
            self.disable_filter_button.setVisible(False)
            self.enable_filter_button.setVisible(False)
    ...
    def _initUI(self):
        ...
        self.active_channels = np.size(
            self.device.get_device_active_channels(), 0
        )
        ...
        for i in range(self.active_channels - 2):
            _checkBox = QtWidgets.QCheckBox(
                chs[self.active_channel_conversion_list[i]][0]
            )
            if i in self._channel_selection:
                _checkBox.setChecked(True)
            self._gridbox.addWidget(_checkBox, i % 25, np.floor(i / 25))
    ...
    def initUI_impedance(self):
        ...
        for i in range(self.active_channels - 2):
            if self.device.channels[i].type == ChannelType.UNI:
                ...
                self._gridbox.addWidget(_checkBox, i % 25, np.floor(i / 25))
                _checkBox.clicked.connect(
                    self.real_time_plot._update_active_channels
                )
    ...
    def get_conversion_list(self):
        if isinstance(self.device, TMSiDevice):
            self.channel_conversion_list = np.arange(
                0, len(self.device.get_device_channels()), dtype=int
            )
        ...
        config_file = join(
            ...
            "TMSiSDK",
            "_resources",
            "HD_EMG_grid_channel_configuration.json",
        )
        ...
        if exists(config_file):
            ...
            print(
                "Couldn't load HD-EMG conversion file, using default channel ordering"
            )
            self.grid_type = "none"

        # Reorder channels to grid ordering, insert alternative channels as well
        if self.grid_type in self.conversion_data:
            self.channel_conversion_list = np.array(
                self.conversion_data[self.grid_type]["channel_conversion"]
            )
            # Add CREF channel
            self.channel_conversion_list = np.insert(
                self.channel_conversion_list, 0, 0
            )
            print("Grid type is", self.grid_type)

            self.active_channel_conversion_list = copy(
            ...
            for ch in range(len(self.active_channel_conversion_list)):
                if (
                    not self.device.channels[ch - offset].name
                    == self.device._config._channels[ch].alt_name
                ):
                    self.active_channel_conversion_list = np.delete(
                        self.active_channel_conversion_list,
                        (self.active_channel_conversion_list == (ch - offset)),
                    )
                    self.active_channel_conversion_list[
                        self.active_channel_conversion_list > (ch - offset)
                    ] = (
            ...
            self.active_channel_conversion_list = np.hstack(
                (
                    self.active_channel_conversion_list,
                    np.arange(
                        len(self.active_channel_conversion_list),
            ...
            self.active_channel_conversion_list = np.arange(
                0, len(self.device.channels), dtype=int
            )
            self.channel_conversion_list = []
            for ch in range(len(self.device._config._channels)):
                if (
                    self.device._config._channels[ch].type == ChannelType.UNI
                    and not "GND" in self.device._config._channels[ch].alt_name
                ):
        ...
        if isinstance(self.device, TMSiDevice):
            if not impedance:
                self.device.start_measurement(
                    ApexMeasurementType.APEX_EEG, thread_refresh=0.03
                )
            else:
                self.device.start_measurement(
                    ApexMeasurementType.APEX_IMPEDANCE
                )
            return
    ...
    def closeEvent(self, event):
        ...
        if isinstance(self.device, TMSiDevice):
            if not self.filter_app:
                ApexSampleDataServer().unregister_consumer(
                    self.device.get_id(),
                    self.real_time_plot.worker.q_sample_sets,
                )
            else:
                ApexSampleDataServer().unregister_consumer(
                    self.device.get_id(),
                    self.filter_app.filter_thread.q_sample_sets,
                )
        else:
            if not self.filter_app:
                sample_data_server.unregisterConsumer(
                    self.device.id, self.real_time_plot.worker.q_sample_sets
                )
            else:
                sample_data_server.unregisterConsumer(
                    self.device.id, self.filter_app.filter_thread.q_sample_sets
                )
        ...
        if self.plotter_format == PlotterFormat.impedance_viewer:
            if self.real_time_plot._disable_channels:
                print(
                    "\nDisable channels:",
                    self.real_time_plot._disable_channels,
                    "\n",
                )
