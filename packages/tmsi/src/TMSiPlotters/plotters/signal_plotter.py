...
class SignalViewer():
    ...
    def initUI(self):
        ...
        self.RealTimePlotWidget.window.setXRange(0.02*self.window_size, 0.97*self.window_size)

        # Create curves and set the position of the curve on the y-axis for each channel
        self.curve = []
        for i in range(self.num_channels):
            self.c = pg.PlotCurveItem()
            self.c.setPen(color = 'black', cosmetic = True, joinStyle = QtCore.Qt.MiterJoin)
    ...
    def _decrease_time_range(self):
        ...
        if self.window_size > 1:
            ...
            self.gui_handle.increase_time_button.setText('Increase time range: ' + str(self.window_size + 1) + 's')
            if self.window_size == 1:
                self.gui_handle.decrease_time_button.setEnabled(False)
            else:
                self.gui_handle.decrease_time_button.setEnabled(True)
                self.gui_handle.decrease_time_button.setText('Decrease time range: ' + str(self.window_size - 1) + 's')
    ...
    def _update_scale(self, type_flag):
        ...
        for i in range(self.num_channels):
            ...
            if type_flag == 'scale':
                ...
                self._plot_diff[i]['diff'] = int(self.gui_handle.set_range_box.currentText())
    ...
    def _update_channel_display(self):
        ...
        for i in range(np.size(self.gui_handle._checkboxes,0)):
            if self.gui_handle._checkboxes[i][0].isChecked():
                if i in self.gui_handle._channel_selection:
                    ...
                    _increase = True
                    self.gui_handle._channel_selection = np.hstack((self.gui_handle._channel_selection,i))
            else:
                if i in self.gui_handle._channel_selection:
                    # Create a flag that states if the number of channels displayed decreases
                    _increase = False
                    self.gui_handle._channel_selection = np.delete(self.gui_handle._channel_selection, self.gui_handle._channel_selection == i)
                else:
                    pass
        ...
        if _increase:
            _idx_overlap = [np.where(i == self.gui_handle._channel_selection)[0][0] for idx,i in enumerate(_idx_curve_list) if i in self.gui_handle._channel_selection]
            ...
            _idx_overlap = [np.where(i == _idx_curve_list)[0][0] for idx,i in enumerate(_idx_curve_list) if i in self.gui_handle._channel_selection]
    ...
    def _show_all_UNI(self):
        ...
        UNI_lst = [i for i in range(np.size(self.gui_handle._checkboxes,0)) if self.gui_handle._checkboxes[i][1].value == ChannelType.UNI.value]
    ...
    def _hide_all_UNI(self):
        ...
        if all(not self.gui_handle._checkboxes[i][0].isChecked() for i in UNI_lst):
            return
    ...
    def _show_all_BIP(self):
        ...
        BIP_lst = [i for i in range(np.size(self.gui_handle._checkboxes,0)) if self.gui_handle._checkboxes[i][1].value == ChannelType.BIP.value]
    ...
    def _hide_all_BIP(self):
        ...
        if all(not self.gui_handle._checkboxes[i][0].isChecked() for i in BIP_lst):
            return
    ...
    def _show_all_AUX(self):
        ...
        AUX_lst = [i for i in range(np.size(self.gui_handle._checkboxes,0)) if self.gui_handle._checkboxes[i][1].value == ChannelType.AUX.value]
    ...
    def _hide_all_AUX(self):
        ...
        if all(not self.gui_handle._checkboxes[i][0].isChecked() for i in AUX_lst):
            return
    ...
    def _show_all_DIGI(self):
        ...
        DIGI_lst = [i for i in range(np.size(self.gui_handle._checkboxes,0)) if self.gui_handle._checkboxes[i][1].value == ChannelType.sensor.value]
        ...
        for i in range(np.size(self.gui_handle._checkboxes,0)):
            if self.gui_handle._checkboxes[i][1].value == ChannelType.sensor.value:
                self.gui_handle._checkboxes[i][0].setChecked(True)
    ...
    def _hide_all_DIGI(self):
        ...
        if all(not self.gui_handle._checkboxes[i][0].isChecked() for i in DIGI_lst):
            return
    ...
    def update_plot(self, data):
        ...
        try:
            for i in range(self.num_channels):
                ...
                self.curve[i].setData(time_axis, (data[self.gui_handle._channel_selection[i],:] - self._plot_diff[i]['mean']) / self._plot_diff[i]['diff'] * -1,
