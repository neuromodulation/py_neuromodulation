...
class HeatMapViewer:
    ...
    def __init__(
        ...
        tail_orientation,
        signal_lim,
        grid_type="flex pcb grid",
    ):
        """Setting up the GUI's elements"""

        if sys.platform == "linux" or sys.platform == "linux2":
            print(
                "This plotter is not compatible with the current version of the TMSi Python Interface on Linux (Ubuntu 18.04 LTS)\n"
            )
            return
    ...
    def initUI(self):
        ...
        self.channel_conversion_list = np.delete(
            self.gui_handle.channel_conversion_list[: self._EMG_chans + 1], 0
        )
        ...
        if "11" in self.grid_type:
            if not self.grid_type[-1] == "2":
                ...
                self.channel_conversion_list = np.insert(
                    ...
                    10,
                    max(self.channel_conversion_list) + 1,
                )
            if not self.grid_type[-1] == "1":
                dummy_chan = max(self.channel_conversion_list) + 1
                self.dummy_chan.append(dummy_chan)
                self.channel_conversion_list = np.append(
                    self.channel_conversion_list, dummy_chan
                )
        ...
        if not "11" in self.grid_type:
            if self._EMG_chans == 32:
                self._x_interpolate = np.arange(
                    0, int(self._EMG_chans / 8) - 1 + 0.1, 0.1
                )
                self._y_interpolate = np.arange(
                    0,
                    int(self._EMG_chans / (self._EMG_chans / 8)) - 1 + 0.1,
                    0.1,
                )
            else:
                self._x_interpolate = np.arange(
                    0, int(self._EMG_chans / 8) - 1 + 0.2, 0.1
                )
                self._y_interpolate = np.arange(
                    0,
                    int(self._EMG_chans / (self._EMG_chans / 8)) - 1 + 0.1,
            ...
            self._x_interpolate = np.arange(0, self.n_x - 1 + 0.1, 0.1)
            self._y_interpolate = np.arange(0, self.n_y - 1 + 0.1, 0.1)
        self._dummy_val = np.zeros(
            (len(self._x_interpolate), len(self._y_interpolate))
        )
        ...
        self.bar = pg.ColorBarItem(
            values=(0, self.signal_lim),
            colorMap=cm,
            interactive=False,
            label="RMS (\u03BCVolt)",
        )
        self.bar.setImageItem(
            self.img, insert_in=self.RealTimePlotWidget.window
        )
        ...
        locs = np.array(
            [
                [(i % self.n_y) * corr_y, int(i / self.n_y) * corr_x]
        ...
        if self.tail_orientation == "Left" or self.tail_orientation == "left":
            self.spots = [
                ...
                for i in range(self._EMG_chans + len(self.dummy_chan))
                if self.channel_conversion_list[i] <= self._EMG_chans
            ]
            tail_text.setPos(
                self.spots[0]["pos"][0] - 1 * corr_y,
                self.spots[0]["pos"][1] + 1.5 * corr_x,
            )

        elif self.tail_orientation == "Up" or self.tail_orientation == "up":
            self.spots = [
                {
                    "pos": [locs.T[1][-i - 1], locs[::-1].T[0][-i - 1]],
                    "size": 5,
                    "pen": "k",
                    "brush": QtGui.QBrush(QtGui.QColor(128, 128, 128)),
                }
            ...
            tail_text.setPos(
                self.spots[0]["pos"][0] - 2 * corr_y,
                self.spots[0]["pos"][1] - 0.5 * corr_x,
            )

        elif (
            self.tail_orientation == "Right" or self.tail_orientation == "right"
        ):
            self.spots = [
                {
                    "pos": locs[::-1][i],
            ...
            self.spots = [
                {
                    "pos": [locs[::-1].T[1][-i - 1], locs.T[0][-i - 1]],
                    "size": 5,
                    "pen": "k",
                    "brush": QtGui.QBrush(QtGui.QColor(128, 128, 128)),
                }
        ...
        for i in range(len(self.device._config._channels)):
            ...
            if i_ch <= (self._EMG_chans):
                ...
                t_item.setPos(
                    self.spots[i - i_corr]["pos"][0] - 0.25,
                    self.spots[i - i_corr]["pos"][1] + 0.1,
                )
...
class SamplingThread(QtCore.QObject):
    ...
    def __init__(self, main_class):
        ...
        self.window_buffer = np.zeros(
            (self._EMG_chans + len(main_class.dummy_chan), self.sample_rate * 5)
        )
        self.window_rms_size = self.sample_rate // 4
        self._add_final = 0

        self.sos = signal.butter(
            2, 10, "highpass", fs=self.sample_rate, output="sos"
        )
        z_sos0 = signal.sosfilt_zi(self.sos)
        self.z_sos = np.repeat(
            z_sos0[:, np.newaxis, :],
            (self._EMG_chans + len(main_class.dummy_chan)),
            axis=1,
        )
        ...
        while self.sampling:
            while not self.q_sample_sets.empty():
                ...
                for ch in self.dummy_chan:
                    samples = np.insert(
                        samples, self._EMG_chans + 1, np.nan, axis=0
                    )

                # Fill the window buffer with the reshaped sample set
                self.window_buffer[
                    :, self._add_final : (self._add_final + self.new_samples)
                ] = samples[conversion_list, :]

                if self._add_final + self.new_samples > self.window_rms_size:
                    filt_data, self.z_sos = signal.sosfilt(
                        self.sos,
                        self.window_buffer[:, 0 : self.window_rms_size],
                        zi=self.z_sos,
                    )
                    ...
                    if self._preprocess_wifi:
                        if np.isnan(filt_data).any():
                            print(
                                "The filter crashed due to lost samples; resetting filter..."
                            )
                            self.sos = signal.butter(
                                2,
                                10,
                    ...
                    f = interpolate.interp2d(
                        self._x_grid, self._y_grid, rms_filled, kind="linear"
                    )
                    f_nan = interpolate.interp2d(
                        self._x_grid, self._y_grid, nan_map, kind="linear"
                    )

                    heatmap = f(self._x_interpolate, self._y_interpolate)
                    nan_heatmap = f_nan(
                        self._x_interpolate, self._y_interpolate
                    )
                    heatmap[nan_heatmap > 0] = np.nan

                    # Rotate heatmap based on tail orientation
                    if (
                        self.tail_orientation == "Left"
                        or self.tail_orientation == "left"
                    ):
                        output_heatmap = heatmap
                    elif (
                        self.tail_orientation == "Up"
                        or self.tail_orientation == "up"
                    ):
                        output_heatmap = np.rot90(heatmap, 1)
                    elif (
                        self.tail_orientation == "Right"
                        or self.tail_orientation == "right"
                    ):
                        output_heatmap = np.rot90(heatmap, 2)
                    elif (
                        self.tail_orientation == "Down"
                        or self.tail_orientation == "down"
                    ):
                        output_heatmap = np.rot90(heatmap, 3)

                    self._add_final = 0

                    self.window_buffer = np.hstack(
                        (
                            self.window_buffer[:, self.window_rms_size :],
                            np.zeros(
                                (
                                    (self._EMG_chans + len(self.dummy_chan)),
                                    self.window_rms_size,
                                )
                            ),
                        )
                    )
...
if __name__ == "__main__":
    ...
    window = HDEMGPlot(
        figurename="An HDEMG Heatmap Plot", device=dev, tail_orientation="up"
    )
    window.show()
