...
def read_BIDS_data(PATH_RUN, BIDS_PATH):
    ...
    Parameters
    ----------
    PATH_RUN : string | mne_bids.BIDSPath
    BIDS_PATH : string
    ...
    """
    try:
        raw_arr = mne_bids.read_raw_bids(PATH_RUN)
    except RuntimeError:
...
def add_labels(df_, settings_wrapper, raw_arr_data):
    """Given a constructed feature data frame, resample the target labels and
    add to dataframe
    ...
    if ind_label.shape[0] != 0:
        ...
        offset_start = np.ceil(
            offset_time / 1000 * settings_wrapper.settings["fs"]).astype(int)
        ...
        if df_.shape[0] == label_downsampled.shape[1]:
            ...
            print("Label dimensions don't match, saving downsampled label "
                  "separately.")
    else:
