...
class RT_rereference:
    ...
    def __init__(self, ch_names, refs, to_ref_idx, cortex_idx, \
                 subcortex_idx, split_data=False) -> None:
        ...
        Args:
            ...
            split_data (bool, optional): If set to True, the rereferenced data will be 
                returned split into cortex and subcortex. Defaults to False.
        """
    ...
    def rereference(self, ieeg_batch) -> np.ndarray:
        ...
        for i, idx in enumerate(self.subcortex_idx):
            ...
            if self.refs[idx] in ['none', 'None'] or pd.isnull(self.refs[idx]):
                ...
                av = np.mean(data_subcortex[self.subcortex_idx != idx, :], axis=0)
        ...
        for i, idx in enumerate(self.cortex_idx):
            ...
            if self.refs[idx] == 'none' or pd.isnull(self.refs[idx]):
                ...
                av = np.mean(data_cortex[self.cortex_idx != idx, :], axis=0)
