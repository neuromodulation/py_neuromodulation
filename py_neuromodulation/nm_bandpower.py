...
def get_bandpower_features(features_, s, seglengths, dat_filtered, KF_dict, ch, ch_idx):
    ...
    features_ : dict
    """
    for idx, f_band in enumerate(s["frequency_ranges_hz"].keys()):
        seglength = seglengths[idx]
        for bp_feature in [
            k
            for k, v in s["bandpass_filter_settings"]["bandpower_features"].items()
            if v is True
        ]:
            if bp_feature == "activity":
                if s["bandpass_filter_settings"]["log_transform"]:
                    feature_calc = np_log(var(dat_filtered[ch_idx, idx, -seglength:]))
                else:
                    feature_calc = var(dat_filtered[ch_idx, idx, -seglength:])
            elif bp_feature == "mobility":
                deriv_variance = var(diff(dat_filtered[ch_idx, idx, -seglength:]))
                feature_calc = sqrt(
                    deriv_variance / var(dat_filtered[ch_idx, idx, -seglength:])
                )
            ...
            feature_name = "_".join([ch, "bandpass", bp_feature, f_band])
