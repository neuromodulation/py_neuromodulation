...
features = ['combined']
# TODO: Select the best performing ch from the other type
features = ['combined']
            ...
            for featureidx in range(len(features)):
                ...
                for runs in overlapruns:
                    x_temp = np.concatenate(
                        (np.squeeze(ch_all[cohort][sub][bestECOG][runs]['data']),
                         np.squeeze(ch_all[cohort][sub][bestLFP][runs]['data'])), axis=1)
                ...
                for i, (train_index, test_index) in enumerate(crossvals):
                    scores.append(RunCebra(x_concat[train_index, :], np.zeros_like(x_concat[train_index, 0]),
                                           y_concat[train_index], x_concat[test_index, :], y_concat[test_index]))
