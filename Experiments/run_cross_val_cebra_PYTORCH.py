...
def run_CV(val_approach,curtime,model_params,show_embedding=False,embeddingconsistency=False,showfeatureweights=False):
    ...
    for cohort_test in cohorts:
        if cohort_test not in p_:
            p_[cohort_test] = {}
        bacohort = []
        for sub_test in train_select[cohort_test].keys(): # test_select for unseen test data
            ...
            if (val_approach == "leave_1_cohort_out" and cohort_test != cohort_prev_it) or val_approach != "leave_1_cohort_out":
                ...
                if embeddingconsistency: # TODO: Fix computation of embedding consistencies
                    ...
                    trainembeddinglabels.append(y_train_discr[neural_model.get_offset().left:-neural_model.get_offset().right+1])
                ...
                if model_params['decoder'] == 'KNN':
                    ...
                    classes_weights = class_weight.compute_sample_weight(
                        class_weight="balanced", y=y_train_discr[neural_model.get_offset().left:-neural_model.get_offset().right+1]
                    )
            ...
            if embeddingconsistency:
                ...
                testembeddinglabels.append(y_test[neural_model.get_offset().left:-neural_model.get_offset().right+1])
