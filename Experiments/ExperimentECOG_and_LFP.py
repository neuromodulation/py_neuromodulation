...
performancedict = {}
for cohort in list(ch_all.keys())[:-1]: # Exclude washington without LFP
    print(cohort)
    ...
    for sub in ch_all[cohort].keys():
        ...
        sortedChannels = df.query(f"cohort == @cohort and sub == @sub").sort_values(
            by='ba_combined', ascending=False)[["type","ch"]]

        for nrchannels in range(1,3):
            chlist = [sortedChannels['ch'].values[0]]
            typeoffirst = sortedChannels['type'].values[0]
            if nrchannels == 2:
                sortedOther = df.query(f"cohort == @cohort and sub == @sub and type != @typeoffirst").sort_values(
                    by='ba_combined', ascending=False)[["type", "ch"]]
                chlist.append(sortedOther['ch'].values[0])
