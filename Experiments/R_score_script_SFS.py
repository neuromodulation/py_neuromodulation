    ...
    for sub in ch_all[cohort].keys():
        ...
        if cohort == 'Berlin' and sub == '001':
            for channel in ch_all[cohort][sub].keys():
                ...
                performancedict[cohort][sub][channel]['sel_features'] = {}
                ...
                for featureidx in range(len(features)):
                    ...
                    if coef:
                        cv_out = cross_validate(pipe, x_concat, np.array(y_concat, dtype=int), cv=kf, scoring = bascorer,return_estimator=True)
                        ...
                        cv_out = cross_validate(pipe, x_concat, np.array(y_concat, dtype=int), cv=kf, scoring = bascorer, return_estimator=True,return_train_score=True)
                    scores = cv_out['test_score']
                    performancedict[cohort][sub][channel]['ba'][features[featureidx]] = np.mean(scores)
                    print(np.mean(scores))
                    performancedict[cohort][sub][channel]['sel_features'][features[featureidx]] = {}
                    selfeatch = []
                    for i in range(3):
                        selfeatch.append(cv_out['estimator'][i].named_steps['selector'].n_features_to_select_) # n_features_ for rfe
                        performancedict[cohort][sub][channel]['sel_features'][features[featureidx]][str(i)] = cv_out['estimator'][i].named_steps['selector'].support_ # ranking_ for rfe
                    nrfeatsel.append(np.mean(selfeatch))
                    print(nrfeatsel)
            ...
            for channel in list(ch_all[cohort][sub].keys())[:-1]:
                ...
                for featureidx in range(len(features)):
                    ...
                    selfeatch = []
                    for i in range(3):
                        selfeatch.append(cv_out['estimator'][i].named_steps['selector'].n_features_to_select_)
                        performancedict[cohort][sub][channel]['sel_features'][features[featureidx]][str(i)] = cv_out['estimator'][i].named_steps['selector'].support_
                    nrfeatsel.append(np.mean(selfeatch))
                    performancedict[cohort][sub][channel]['ba'][features[featureidx]] = np.mean(scores)
                    performancedict[cohort][sub][channel]['95%CI'][features[featureidx]] = np.std(scores)*2
                performancedict[cohort][sub][channel]['explength'] = len(y_concat)
            ...
            for channel in list(ch_all[cohort][sub].keys()):
                ...
                try: # Just to get around the one channel that does not have MedOn
                    for featureidx in range(len(features)):
                        ...
                        for i in range(3):
                            selfeatch.append(cv_out['estimator'][i].named_steps['selector'].n_features_to_select_)
                            performancedict[cohort][sub][channel]['sel_features'][features[featureidx]][str(i)] = \
                            cv_out['estimator'][i].named_steps['selector'].support_
                        nrfeatsel.append(np.mean(selfeatch))
                        performancedict[cohort][sub][channel]['ba'][features[featureidx]] = np.mean(scores)
                        performancedict[cohort][sub][channel]['95%CI'][features[featureidx]] = np.std(scores)*2
                    performancedict[cohort][sub][channel]['explength'] = len(y_concat)
...
np.save(r'C:\Users\ICN_GPU\Documents\Glenn_Data\SequentialFeatureSelections.npy', performancedict)
if coef:
