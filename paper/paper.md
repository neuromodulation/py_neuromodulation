...
To simplify electrophysiological analysis and decoding, we added a react frontend application, which runs within an ASGI `uvicorn <https://www.uvicorn.org/>`_ server and communicates through `FastAPI <https://fastapi.tiangolo.com/>`_. Here the same settings as in the Python backend can be modified (\autoref{fig:settings}). The stream can be selected from an offline file or LabStreamingLayer stream. 
